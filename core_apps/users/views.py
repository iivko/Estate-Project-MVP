...
def set_auth_cookies(response: Response, access_token: str, refresh_token: Optional[str] = None) -> None:
    ...
    response.set_cookie("logged_in", "true", **logged_in_cookie_settings) # Only show status logged in. No sensitive data
    ...
    def post(self, request: Request, *args, **kwargs) -> Response:
        ...
        if token_response.status_code == status.HTTP_200_OK:
            ...
            if access_token and refresh_token:
                set_auth_cookies(token_response, access_token=access_token, refresh_token=refresh_token)
...
class CustomTokenRefreshView(TokenRefreshView):
    def post(self, request: Request, *args, **kwargs) -> Response:
        ...
        if refresh_response.status_code == status.HTTP_200_OK:
            ...
            if access_token and refresh_token:
                set_auth_cookies(refresh_response, access_token=access_token, refresh_token=refresh_token)
...
class CustomProviderAuthView(ProviderAuthView):
    def post(self, request: Request, *args, **kwargs) -> Response:
        ...
        if provider_response.status_code == status.HTTP_201_CREATED:
            ...
            if access_token and refresh_token:
                set_auth_cookies(provider_response, access_token=access_token, refresh_token=refresh_token)
