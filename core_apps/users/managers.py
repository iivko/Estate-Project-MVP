...
def validate_email_address(value: str):
    try:
        ...
        raise ValidationError(_("The email address is not valid. Enter a valid email address."))
...
class UserManager(DjangoUserManager):
    def _create_user(self, username: str, email: str, password: str | None, **extra_fields):
        ...
        # safely fetching a model class from the app global registry, instead of directly importing it.
        global_user_model = apps.get_model(
            self.model._meta.app_label, # the name of the app where the model lives.
            self.model._meta.object_name # the model class name
        )
