...
class PostUpdateAPIView(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        post_with_replies_count = Post.objects.annotate(replies_count=Count("replies")).get(pk=self.post_instance.pk)
...
class UnBookmarkPostAPIView(APIView):
    def patch(self, request, slug):
        ...
        if user not in post.bookmarked_by.all():
            return Response(
                {"message": "You can't remove bookmark that did not exist in the first place."},
                status=status.HTTP_400_BAD_REQUEST
            )
...
class UpvotePostAPIView(APIView):
    def patch(self, request, post_id):
        post = get_object_or_404(Post, id=post_id)
        serializer = UpvotePostSerializer(post, data=request.data, context={"request": request})

        if serializer.is_valid():
            serializer.save()
            return Response({"message": "Post upvoted successfully."}, status=status.HTTP_200_OK)
...
class DownvotePostAPIView(APIView):
    def patch(self, request, post_id):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({"message": "Post downvoted successfully."}, status=status.HTTP_200_OK)
...
class TopPostListAPIView(generics.ListAPIView):
...
class PostsByTagListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return Post.objects.filter(tags__slug=tag_slug).annotate(replies_count=Count("replies"))