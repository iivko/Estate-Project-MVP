...
def send_issue_confirmation_email(issue: Issue) -> None:
    try:
        ...
        logger.error(f"Failed to send issue confirmation email for issue '{issue.title}': {str(e)}", exc_info=True)
...
def send_issue_resolved_email(issue: Issue) -> None:
    try:
        ...
        logger.error(f"Failed to send resolution email for issue '{issue.title}': {str(e)}", exc_info=True)
