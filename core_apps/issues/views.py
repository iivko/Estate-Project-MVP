...
class AssignedIssueListAPIView(generics.ListAPIView):
    serializer_class = IssueSerializer
    renderer_classes = [GenericJSONRenderer]
...
class IssueCreateAPIView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer: IssueSerializer) -> None:
        ...
        try:
            ...
            raise PermissionDenied("You do not have permission to report an issue for this apartment. It's not yours.")
...
class IssueDetailAPIView(generics.RetrieveAPIView):
    ...
    def get_object(self) -> Issue:
        ...
        if not (
            user == issue.reported_by or user.is_staff or user.is_assigned_to
        ):
